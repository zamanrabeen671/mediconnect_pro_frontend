...
export const Qualification = () => {
  ...
  const [filteredQualifications, setFilteredQualifications] = useState<QualificationType[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [showEditModal, setShowEditModal] = useState(false);
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [editingQualification, setEditingQualification] = useState<QualificationType | null>(null);
  const [editFormData, setEditFormData] = useState<QualificationType>({});
  const [createFormData, setCreateFormData] = useState<QualificationType>({ name: "" });
  ...
  const fetchQualifications = async () => {
    setListLoading(true);
    try {
      const result = await dispatch(getQualificationList(null));
      if (result.payload) {
        const quals = Array.isArray(result.payload) ? result.payload : [];
  ...
  useEffect(() => {
    ...
    const filtered = qualifications.filter((qual: QualificationType) =>
  ...
  const currentQualifications = filteredQualifications.slice(startIndex, endIndex);
  ...
  // Handle edit qualification - open modal
  const handleEditClick = (qualification: QualificationType) => {
    setEditingQualification(qualification);
    setEditFormData({ ...qualification });
  ...
  const handleUpdateQualification = async (e: React.FormEvent) => {
    ...
    if (!editingQualification?.id || !editFormData.name?.trim()) {
    ...
    try {
      ...
      await dispatch(updateQualification({ qualificationId: editingQualification.id, payload }));
      ...
      setEditError(err.message || "Failed to update qualification");
    } finally {
      setEditLoading(false);
    }
  };

  // Handle create qualification submission
  const handleCreateQualification = async (e: React.FormEvent) => {
    e.preventDefault();
    setCreateError("");

    if (!createFormData.name?.trim()) {
      setCreateError("Qualification name is required");
      return;
    }
    ...
    try {
      ...
      await dispatch(createQualification(payload));
      await fetchQualifications();

      setShowCreateModal(false);
      setCreateFormData({ name: "" });
    } catch (err: any) {
      setCreateError(err.message || "Failed to create qualification");
    } finally {
  ...
  const handleDeleteQualification = async (qualificationId: number | undefined) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <p className="text-muted-foreground mt-1">Manage qualifications in the system</p>
        </div>
        <button 
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        {listLoading ? (
          ...
          <div className="p-8 text-center text-muted-foreground">
            {qualifications.length === 0 ? "No qualifications found. Add one to get started." : "No qualifications match your search."}
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {currentQualifications.map((qualification: QualificationType, index: number) => (
                  <tr
                    key={qualification.id || index}
                    className="border-b border-border hover:bg-muted/50 transition-colors"
                  >
                    <td className="px-6 py-4 text-sm text-foreground">{qualification.id}</td>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="bg-muted p-4 rounded-lg text-sm text-muted-foreground flex justify-between items-center">
          <div>
            {filteredQualifications.length > 0 ? (
              <span>
                Showing <strong>{startIndex + 1}</strong> to <strong>{Math.min(endIndex, filteredQualifications.length)}</strong> of <strong>{filteredQualifications.length}</strong> qualifications
              </span>
            ) : (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-100 border border-border rounded-lg shadow-lg max-w-md w-full">
            ...
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h3 className="text-xl font-bold text-foreground">Create Qualification</h3>
              <button
              ...
              <form onSubmit={handleCreateQualification} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., MD (Doctor of Medicine)"
      ...
      {showEditModal && editingQualification && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-100 border border-border rounded-lg shadow-lg max-w-md w-full">
            {/* Modal Header */}
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h3 className="text-xl font-bold text-foreground">Edit Qualification</h3>
              <button
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleUpdateQualification} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">
                    Qualification Name *
                  </label>
