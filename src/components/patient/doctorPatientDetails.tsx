...
export const DcotorPatientDetails: React.FC = () => {
  ...
  useEffect(() => {
    ...
    dispatch(getPatientAppointment(Number(patientId)))
  }, [patientId, dispatch])
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-4 mt-3 text-sm text-muted">
                  ...
                  <div className="ml-4"><span className="px-3 py-1 bg-muted rounded-full text-sm">{patient?.blood_group?.group_name || "-"}</span></div>
                </div>
              </div>
            </div>
                        ...
                        {patientAppointments.map((a: any) => (
                          <li key={a.id} className="p-3 bg-muted/10 rounded-md flex justify-between items-center">
                            <div>
                              ...
                              <div className="text-sm text-muted">{a.appointment_date + (a.appointment_time ?? '')}</div>
                            </div>
