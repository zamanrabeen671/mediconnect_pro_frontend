...
import { doctorSchedule, getDoctorAppointmentState, getDoctorStates } from "../../store/API/doctorApi"

export default function DoctorDashboard() {
  const dispatch = useAppDispatch()
  const { user } = useAppSelector((state) => state.auth)
  const { schedule } = useAppSelector((state) => state.doctor)
  const { appointmentState, dashboardState } = useAppSelector(state => state.doctor);
  ...
  useEffect(() => {
    if(user?.id) {
      ...
      dispatch(getDoctorAppointmentState(user.id))
      dispatch(getDoctorStates(user.id))
    }
  }, [dispatch])
        ...
        <div className="flex-1 px-8 py-8 overflow-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            <div className="bg-card border border-border rounded-xl p-6">
              ...
              <div className="text-3xl font-bold text-foreground mb-1">{dashboardState?.today_appointments || 0}</div>
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              ...
              <div className="text-3xl font-bold text-foreground mb-1">{dashboardState?.total_patients || 0}</div>
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              ...
              <div className="text-3xl font-bold text-foreground mb-1">{dashboardState?.pending_reports || 0}</div>
              ...
              {appointmentState?.map((item, index) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="font-medium text-foreground">Patient {item?.patient?.full_name}</div>
                      <div className="text-sm text-muted-foreground">Regular Checkup</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-medium text-foreground">{item?.appointment_time}</div>
