...
const PurchaseInvoice: React.FC<ProductInvoiceProps> = ({ purchase }) => {
  return (
    <div style={styles.container}>
      ...
      <div style={styles.details}>
        <div>
          <p>Date: 21-Feb-25</p>
          <p>Name: {purchase?.supplier}</p>
        </div>
        <div>
          <p>Invoice No: {purchase?.invoiceNumber}</p>
          <p>Phone No: {purchase?.supplierPhone}</p>
        </div>
      </div>
      ...
      <table style={styles.table}>
        ...
        <tbody>
          {purchase?.purchaseProducts &&
            ...
            purchase?.purchaseProducts.map((item, index) => (
      ...
      <div style={styles.totalSection}>
        <div style={styles.row}>
          ...
          <div style={styles.amount}>৳{purchase?.price}</div>
        </div>
        <div style={styles.row}>
          <div style={styles.label}>Paid </div>
          <div style={styles.amount}>৳{purchase?.paidAmount || 0}</div>
        </div>
        <div style={styles.row}>
          <div style={styles.label}>Due</div>
          <div style={styles.amount}>৳{(Number(purchase?.price) - Number(purchase?.paidAmount)) || 0}</div>
        </div>
      </div>
    </div>
  );
